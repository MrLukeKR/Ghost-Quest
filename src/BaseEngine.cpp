...
void BaseEngine::CreateObjectArray(int iNumberObjects)
{
	...
	DestroyOldObjects();
	objects.resize(iNumberObjects);
}



void BaseEngine::StoreObjectInArray(int index, DisplayableObject* pObject)
{
	if (pObject != NULL)
	StoreObjectInArray(pObject);
}
...
void BaseEngine::RemoveDisplayableObject(DisplayableObject* object){
	int copyPoint = 0;
	DisplayableObject* delObj;
	int newSize = objects.size() - 1;
	...
	delete objects[copyPoint];

	for (int i = copyPoint; i < objects.size(); i++)
...
int BaseEngine::GetNumberOfObjects() { 
	return objects.size();
}
...
{
	DisplayableObject* currObj;
	int objectCount = objects.size();
	m_iDrawableObjectsChanged = 0;

	for (int i = 0; i < objectCount; i++){

		if (m_iDrawableObjectsChanged || objectCount != objects.size())
			return;

		currObj = objects.at(i);

		if (currObj == NULL)
			return;

		currObj->DoUpdate(iCurrentTime);
	}
}
...
void BaseEngine::NotifyAllObjects( int iSignalNumber )
{
	if ( &objects != NULL )
		for ( int i = 0; i <objects.size(); i++ )
			objects.at(i)->Notify(iSignalNumber);
}
...
int BaseEngine::NotifyAllObjectsGetCountNonZero( int iSignalNumber )
{
	...
	if ( &objects != NULL )
	{
		for (int i = 0; i < objects.size(); i++)
		{
			if ( objects.at(i)->Notify( iSignalNumber ) != 0 )
				iReturn++;
		}
	}
	return iReturn;
...
int BaseEngine::NotifyAllObjectsGetSum( int iSignalNumber )
{
	...
	if ( &objects != NULL )
	{
		...
		{
			iReturn += objects.at(i)->Notify( iSignalNumber );
		}
	}
	return iReturn;
}
...
int BaseEngine::NotifyAllObjectsGetMax( int iSignalNumber )
{
	...
	if ( &objects != NULL )
	{
		for ( int i = 0; i <GetNumberOfObjects(); i++ )
		{
			int ival = objects.at(i)->Notify( iSignalNumber );
			if ( ival > iReturn )
				iReturn = ival;
		}
	}
